...
- **CORS on remote result URLs**: when an AI provider returns a result file as a hosted `item.url` (instead of inline `b64_json`), persisting it to IndexedDB requires `fetch(url).blob()`, which the browser blocks if that CDN sends no `Access-Control-Allow-Origin`. First line of defense is requesting `response_format: "b64_json"` (already done for images); when only a URL is available, the local canvas-agent acts as a same-machine proxy via `POST /agent/fetch { url }` (Node fetch, not subject to browser CORS), guarded by the token + origin allow-list, 30s timeout, 30MB cap, http(s)-only. Web side routes these through `fetchBlob()` in `web/src/services/blob-fetch.ts` (used by `image-storage`, `file-storage`, `audio` plugin path); when the agent is not connected it falls back to the dev-server `/api/fetch-blob` proxy (vite.config.ts `localFetchProxy()`, dev/preview only), then direct `fetch`.
- **Dev-only AI request proxy**: some providers (e.g. Volcengine Ark) don't send CORS headers on their API endpoints, so browser-direct generation requests are blocked. In dev, `installAiProxy()` (`web/src/services/api/ai-proxy.ts`, called in `main.tsx`) rewrites axios request URLs and `aiFetch()` (used for the SSE streaming calls in `image.ts`) through the Vite dev server's `/api/ai-proxy` middleware (vite.config.ts `aiProxy()`), which forwards method/headers/body server-side and streams status/headers/body back same-origin. It only allows public http(s) targets (loopback/link-local/metadata blocked; RFC1918 LAN relays allowed). Production static deploys still need the canvas-agent or a CORS-capable relay for such providers.
